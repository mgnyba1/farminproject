...
public class Program {
    ...
    public static void run(String[] args) throws IOException, ExecutionControl.NotImplementedException {
        ...
        System.out.println("FARMA projekt Mateusz Gnyba");
    ...
    public static void showAvailableActionsDialog() throws IOException, ExecutionControl.NotImplementedException {
        ...
        switch(choosenNumber)
        {
                ...
                switch(writtenNumber)
                {
                    ...
                    case 1:
                        ...
                        else
                        {
                            ...
                            if(Tools.lessThan(player.money, animalWhoWannaBuy.buyCost)) {
                    ...
                    case 2:
                        ...
                        if(writtenNumber == 0)
                            ...
                            if (Tools.lessThan(player.money, seedCostx100)) {
            ...
            case 5: //5. Posadzenie roślin (jeżeli posiadasz sadzonki, które można posadzić w tym okresie)
                ...
                BigDecimal costOfPlanting = selectedSeeds.get(0).costOfLandPrepareAndPlanting.multiply(new BigDecimal(writtenNumber));
                costOfPlanting = costOfPlanting.setScale(2, RoundingMode.HALF_EVEN);

                if(Tools.lessThan(player.money, costOfPlanting))
                {
            ...
            case 6: //6. Zbiory roślin (jeżeli masz gotowe do zebrania plony)
                ...
                for(Building b : player.farms.get(farmIndexWithWareHouse).buildings)
                {
                    if(b.name.contains("todo")) //Stodoła
                    {
                        ((Barn)b).storedPlants.addAll(player.arableLands.get(arableLandIndex).plants);
            ...
            case 7: //7. Sprzedaż roślin lub zwierząt
                ...
                switch(writtenNumber)
                {
                    case 1:
                        ...
                        for(Plant x : ((Barn)player.farms.get(farmIndexWithMainWarehouse).buildings.get(mainWarehouseIndex)).storedPlants)
                        {
                            if(x.weeksWithPossibilityToPlant.contains(currentWeek) == false)
                                continue;
                        ...
                        Iterator<Plant> i = ((Barn)player.farms.get(farmIndexWithMainWarehouse).buildings.get(mainWarehouseIndex)).storedPlants.iterator();
                        while (i.hasNext()) {
                            Plant s = i.next();
    ...
    {
        if(Tools.lessThan(player.money, startingArableLands.get(index).buyCost))
        {
    ...
    public static boolean givePlayerStartingFarm(int farmId)
    {
        ...
        if(Tools.lessThan(player.money, startingFarms.get(farmId).buyCost))
        {
    ...
    public static boolean buyPlayerBuilding(int buildingId) throws IOException {
        ...
        if(Tools.lessThan(player.money, availableBuildingsToBuy.get(buildingId).cost))
        {
